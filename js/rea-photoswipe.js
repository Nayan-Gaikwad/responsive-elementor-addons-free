...
__webpack_require__(12)(IteratorPrototype, __webpack_require__(19)('iterator'), function () { return this; });
...
var _typeof2 = _interopRequireDefault(__webpack_require__(85));

var _parseInt2 = _interopRequireDefault(__webpack_require__(130));
...
(function ($, window, document, undefined) {
  ...
  $.extend(JQPhotoSwipe.prototype, {
    init: function init() {
      this.$element.find(this.settings.target).each(this._registerSlide.bind(this)); // Parse URL and open gallery if it contains #&pid=3&gid=1
    ...
    _registerSlide: function _registerSlide(index, item) {
      var $item = $(item),
          slide = {
    ...
    _getVideoHtml: function _getVideoHtml(url) {
      ...
      if (url.match(/(youtu.be)/) || url.match(/(youtube.com)/)) {
        videoEmbedLink = "//www.youtube.com/embed/" + cleanVideoID + "?autoplay=" + this.settings.autoplay + "";
      }

      if (url.match(/(vimeo.com\/)+[0-9]/) || url.match(/(vimeo.com\/)+[a-zA-Z]/)) {
        videoEmbedLink = "//player.vimeo.com/video/" + cleanVideoID + "?autoplay=" + this.settings.autoplay + "";
      }
    ...
    _thumbnailBounds: function _thumbnailBounds(index) {
    ...
    _photoswipeParseHash: function _photoswipeParseHash() {
      ...
      if (params.gid) {
        params.gid = (0, _parseInt2.default)(params.gid, 10);
      }

      return params;
    },
    _openPhotoSwipe: function _openPhotoSwipe(index, fromURL) {
      var gallery,
      ...
      if (fromURL) {
        ...
        options.index = (0, _parseInt2.default)(index, 10) - 1;
      } else {
        options.index = (0, _parseInt2.default)(index, 10);
    ...
    _photoswipeListen: function _photoswipeListen(gallery) {
      gallery.listen("beforeChange", function () {
        var $allItems = $(this.container).find(".pswp__video"); // Remove active class from all items

        $allItems.removeClass("active"); // Add active class too current open item

        $(this.currItem.container).find(".pswp__video").addClass("active"); // Check all items
...
(function ($, window, document, undefined) {
  ...
  window.REAinInitElements = function ($scope) {
    $scope = $scope || $(document); // Init Carousel and Lightbox
...
(function ($, window, document, undefined) {
  var $vcWindow,
      $__window = $(window); // Add js callback for customizer partials trigger
  ...
  $__window.on('vc_reload', function () {
    // Main selector
    $vcWindow = $('#vc_inline-frame', window.parent.document).contents().find('.vc_element'); // Init photoswipe modules
...
/***/ })
/******/ ]);
//# sourceMappingURL=rea-photoswipe.js.map